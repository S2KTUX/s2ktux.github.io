...
class TerminalSimulationClient{
  ...
  async connect(){
    ...
    try{
      ...
      worker.addEventListener('message',event=>this.receive(event.data));worker.addEventListener('error',event=>this.disconnect({reason:'error',message:event.message||'Excepción no controlada en el Worker'}));
      this.worker=worker;this.kind='worker';await this.request(WORKER_OPERATIONS.READY,{});this.startHealthcheck();
  ...
  disconnect(details={}){
    ...
    this.pending.clear();
    if(wasConnected)this.emit('runtime.disconnected',details);
  }
  receive(message){
