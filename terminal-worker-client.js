...
class TerminalSimulationClient{
  ...
  disconnect(details={}){
    ...
    if(wasConnected&&details.recover!==false&&this.everConnected&&['linux','docker','kubernetes'].includes(this.mode))this.scheduleRestart();
  }
  receive(message){
  ...
  initializeLinux(payload){return this.request(WORKER_OPERATIONS.LINUX_INIT,payload);}
  executeLinux(payload){return this.request(WORKER_OPERATIONS.LINUX_EXECUTE,payload);}
