...
const CORE=['./','index.html','cursos.html','curso.html','leccion.html','terminal.html','terminal-bootstrap.js','site-shell.css','proyectos.html','sobre.html','404.html','favicon.png','assets/og.png','assets/icon-192.png','assets/icon-512.png','courses-data.js','support.js','manifest.webmanifest'];
...
self.addEventListener('fetch',e=>{ const req=e.request; if(req.method!=='GET') return; const url=new URL(req.url); const isDoc = req.mode==='navigate' || (url.origin===location.origin && /\.(html|js|css|webmanifest|json)$/.test(url.pathname));
  if(url.origin===location.origin && isDoc){
    ...
    e.respondWith(fetch(req).then(res=>{ const cp=res.clone(); caches.open(C).then(c=>{ try{c.put(req,cp);}catch(_){}}); return res; }).catch(()=> caches.match(req).then(h=> h || (req.mode==='navigate' ? caches.match('index.html') : Response.error()))));
  } else {
